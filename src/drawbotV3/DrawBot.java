...
import drawbotV3.DrawPointProvider;
import drawbotV3.KeyHandling;
import drawbotV3.LineSprite;
import drawbotV3.Machine;
import drawbotV3.Motor;
import drawbotV3.MotorInstructions;
import drawbotV3.MotorSprite;
...
//		cgg.draw(curPointEll);
	...
	private void drawAllPoints(Graphics2D g){
		ArrayList<CoordVelocity> cvs = drawPointProvider.getCoordVels();
		Ellipse2D.Double el;
		g.setPaint(Color.GRAY);
		for (CoordVelocity cv : cvs) {
			Pointt lo = cv.getCoord();
			el = new Ellipse2D.Double(lo.x, lo.y, .3,.3);
			g.draw(el);
		}
	}
	...
	{
		if (canvas == null) {
			canvas = createImage(Settings.MachineWidth,CanvasHeight ) ;
			cg = canvas.getGraphics();
		}
		cg.setColor(Color.BLACK);
		cg.fillRect(0, 0, Settings.MachineWidth, CanvasHeight );
	}
