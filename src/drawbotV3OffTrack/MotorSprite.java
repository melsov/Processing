...
public class MotorSprite 
{
	...
	public void drawMotor(Graphics2D g, Pointt loc) {
		Ellipse2D.Double gear = new Ellipse2D.Double(loc.x, loc.y, motor.spoolDiameter, motor.spoolDiameter);
		g.setPaint(Color.BLUE);
		g.fill(gear);
		Pointt center = loc.plus(new Pointt(motor.spoolDiameter, motor.spoolDiameter).multiply(.5));
		double a = motor.totalRotationRadians();
		if (isRightSideMotor) a *= -1;
		Pointt radarLoc = new Pointt(motor.spoolDiameter * .5 * Math.cos(a), motor.spoolDiameter * .5 * Math.sin(a));
		...
		pushDownY +=20;
		drawTextAt(g, loc, "chord length: " + motor.stringLength(), pushDownY );
		pushDownY +=20;
