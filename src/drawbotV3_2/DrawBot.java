...
//		drawDebugSomePoints(g2);
		...
		drawTextAt(g2,  new Pointt(Settings.MACHINE_WIDTH - 40, CanvasHeight - 40), percDone ,0, Color.white);
		drawMotorsAndChords(g2);
		graphMotorVelocities(g2, currentMotorInstructions);
		
		if (isTestClient) {
			drawTextAt(g2,new Pointt(Settings.MACHINE_WIDTH *.4f,  20), "TEST CLIENT", 0, Color.YELLOW );
	...
	private void drawMotorsAndChords(Graphics2D g2) {
		...
		motorSpriteR.drawMotor(g2, rightPos.minus(new Pointt(Settings.spoolDiameter,0)));
	}
...
//			device.setFullScreenWindow(fr);
		...
		db.setSize(Settings.MACHINE_WIDTH + lowerInfoWidth, CanvasHeight + lowerInfoHeight);
		fr.add(db);
		fr.setVisible(true);
...
//		machine.doKeyPressed(e);
	...
	private void fillInfoArea(Graphics2D g) {
		...
		g.draw(new Rectangle2D.Double(0, CanvasHeight, Settings.MACHINE_WIDTH, lowerInfoHeight ));
	}
