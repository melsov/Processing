...
//		resetRightHalf(rs);
	...
	public String toInstructionString() {
		...
		sb.append(leftSteps());
		sb.append(':');
		sb.append(rightSteps());
		sb.append(':');
		sb.append(FormatFloat.FloatStringOneDecimal(leftSpeed));
		sb.append(':');
		sb.append(FormatFloat.FloatStringOneDecimal(rightSpeed));
		sb.append(':');
		B.bugln(sb.toString());
		return sb.toString();
	}
	public static int ByteArrayLength() { return 6; } 
	public byte[] toByteArray() {
		byte[] result = new byte[ByteArrayLength()];
		result[0] = (byte) leftSteps();
		result[1] = (byte) rightSteps();
		byte[] bpairLeftSpeed = ConvertToBytes.SignedBytePairFromInt((int) leftSpeed);
		result[2] = bpairLeftSpeed[0];
		result[3] = bpairLeftSpeed[1];
		byte[] bpairRightSpeed = ConvertToBytes.SignedBytePairFromInt((int) rightSpeed);
		result[4] = bpairRightSpeed[0];
		result[5] = bpairRightSpeed[1];
		return result;
	}
	public void debug() {
